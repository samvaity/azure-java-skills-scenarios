...
Create a small Java 17 Maven project that implements a feature flag and configuration management system backed by Azure App Configuration. The project needs:

- A **configuration service class** that retrieves settings from App Configuration. It should support fetching a single setting by key, fetching a setting with a specific label (to distinguish between environments like "production" vs "staging"), and listing all settings that match a key prefix (returned as a map). It should also support conditional reads — if the setting hasn't changed since the last read (based on its last-modified state), the service should avoid re-downloading the value and return the cached version. This is important for reducing unnecessary network traffic when polling for config changes.
...
- A **Main class** that demos: connecting to App Configuration (endpoint from environment variable, authenticated with managed identity), reading some config values with labels, evaluating feature flags for a few sample user IDs with percentage rollout, and starting the config watcher to detect a change.
